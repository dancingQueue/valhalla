 ...
 * roads.
 */
class MagentaCost : public DynamicCost {
 public:
  /**
   * Construct auto costing. Pass in configuration using property tree.
   * @param  config  Property tree with configuration/options.
   */
  MagentaCost(const boost::property_tree::ptree& config);
...
// Constructor
MagentaCost::MagentaCost(const boost::property_tree::ptree& pt)
...
// Destructor
MagentaCost::~MagentaCost() {
}

// Does the costing method allow multiple passes (with relaxed hierarchy
// limits).
bool MagentaCost::AllowMultiPass() const {
  return true;
}

// Get the access mode used by this costing method.
uint32_t MagentaCost::access_mode() const {
  return kAutoAccess;
}

// Check if access is allowed on the specified edge.
bool MagentaCost::Allowed(const baldr::DirectedEdge* edge,
                       const EdgeLabel& pred,
                       const baldr::GraphTile*& tile,
                       const baldr::GraphId& edgeid) const {
...
bool MagentaCost::AllowedReverse(const baldr::DirectedEdge* edge,
               const EdgeLabel& pred,
               const baldr::DirectedEdge* opp_edge,
               const baldr::GraphTile*& tile,
               const baldr::GraphId& opp_edgeid) const {
...
bool MagentaCost::Allowed(const baldr::NodeInfo* node) const  {
  return (node->access() & kAutoAccess) && !(
      node->latlng().lng() > left && node->latlng().lng() < right &&
      node->latlng().lat() > bottom && node->latlng().lat() < top
  );
}

// Get the cost to traverse the edge in seconds
Cost MagentaCost::EdgeCost(const DirectedEdge* edge) const {
...
Cost MagentaCost::TransitionCost(const baldr::DirectedEdge* edge,
                              const baldr::NodeInfo* node,
                              const EdgeLabel& pred) const {
...
Cost MagentaCost::TransitionCostReverse(const uint32_t idx,
                                     const baldr::NodeInfo* node,
                                     const baldr::DirectedEdge* pred,
                                     const baldr::DirectedEdge* edge) const {
  // Accumulate cost and penalty
